...
namespace {
    ...
    inline std::unordered_map<std::string, int> makeNameToDIK() {
        ...
        m["leftalt"] = 0x38;  m["lalt"] = 0x38;
        m["rightalt"] = 0xB8; m["ralt"] = 0xB8;
        m["leftctrl"] = 0x1D; m["lctrl"] = 0x1D;
        m["rightctrl"] = 0x9D; m["rctrl"] = 0x9D;
        m["leftshift"] = 0x2A; m["lshift"] = 0x2A;
        m["rightshift"] = 0x36; m["rshift"] = 0x36;
        // arrows
        m["up"] = 0xC8; m["down"] = 0xD0; m["left"] = 0xCB; m["right"] = 0xCD;
        // numpad / enter
        m["numpadenter"] = 0x9C; m["enter"] = 0x1C; m["return"] = 0x1C;
        // function keys
        m["f1"]=0x3B; m["f2"]=0x3C; m["f3"]=0x3D; m["f4"]=0x3E; m["f5"]=0x3F; m["f6"]=0x40;
        m["f7"]=0x41; m["f8"]=0x42; m["f9"]=0x43; m["f10"]=0x44; m["f11"]=0x57; m["f12"]=0x58;
        return m;
    }
    ...
    inline bool tokenToDIKModifier(const std::string& tok, int& outDIK, std::string& outKind) {
        ...
        if (tok == "leftalt" || tok == "lalt")   { outDIK = it->second; outKind = "lalt"; return true; }
        if (tok == "rightalt" || tok == "ralt")  { outDIK = it->second; outKind = "ralt"; return true; }
        if (tok == "leftctrl" || tok == "lctrl") { outDIK = it->second; outKind = "lctrl"; return true; }
        if (tok == "rightctrl"|| tok == "rctrl") { outDIK = it->second; outKind = "rctrl"; return true; }
        if (tok == "leftshift"|| tok == "lshift"){ outDIK = it->second; outKind = "lshift"; return true; }
        if (tok == "rightshift"|| tok == "rshift"){ outDIK = it->second; outKind = "rshift"; return true; }
        return false;
    }
    ...
    inline int tokenToDIKMain(const std::string& tok) {
        ...
        if (tok.size() > 2 && tok[0] == '0' && (tok[1] == 'x' || tok[1] == 'X')) {
            try { return std::stoi(tok, nullptr, 16); } catch (...) { return 0; }
        }
        // decimal literal
        const bool allDigit = !tok.empty() && std::all_of(tok.begin(), tok.end(), [](unsigned char c){ return std::isdigit(c) != 0; });
        if (allDigit) {
            try { return std::stoi(tok, nullptr, 10); } catch (...) { return 0; }
        }
        return 0;
    }
...
// causes your unresolved externals), we **do not** query modifier
// state via BSKeyboardDevice/BSInputDevice here. For your test binds,
...
std::string toString(const KeyCombo& kc) {
    std::string out;
    auto add = [&](std::string s) { if (!out.empty()) out += '+'; out += std::move(s); };
    ...
    switch (kc.main) {
        case 0x1C: add("Enter"); break;
        case 0x9C: add("NumPadEnter"); break;
        case 0xC8: add("Up"); break;
        case 0xD0: add("Down"); break;
        case 0xCB: add("Left"); break;
        case 0xCD: add("Right"); break;
        default:   add("DIK(" + std::to_string(kc.main) + ")"); break;
    }
    return out;
}
